...
public class DanhMucService extends BasicService<DanhMuc> {
	...
	public List<DanhMuc> getDanhMucCon(DanhMuc danhMuc) {
		...
		if (!danhMuc.getTrangThai().equalsIgnoreCase(core().TT_DA_XOA)) {
			...
			System.out.println(danhMuc.getNode().getChildren());
			for (TreeNode<DanhMuc> el : danhMuc.getNode().getChildren()) {
				System.out.println(el.getData().getName());
				list.add(el.getData());
	...
	public DanhMuc getChuDeTinTuc() {
		ThamSo objTS = find(ThamSo.class).where(QThamSo.thamSo.trangThai.ne(core().TT_DA_XOA))
				.where(QThamSo.thamSo.ma.eq(ThamSoEnum.CAT_ID_TINTUC)).fetchOne();
		DanhMuc objDM = find(DanhMuc.class).where(QDanhMuc.danhMuc.trangThai.ne(core().TT_DA_XOA))
				.where(QDanhMuc.danhMuc.id.eq(objTS.getValue())).fetchOne();
		return objDM;
	}
	
	public DanhMuc getChuDeGioiThieu() {
		ThamSo objTS = find(ThamSo.class).where(QThamSo.thamSo.trangThai.ne(core().TT_DA_XOA))
				.where(QThamSo.thamSo.ma.eq(ThamSoEnum.CAT_ID_GIOITHIEU)).fetchOne();
		DanhMuc objDM = find(DanhMuc.class).where(QDanhMuc.danhMuc.trangThai.ne(core().TT_DA_XOA))
				.where(QDanhMuc.danhMuc.id.eq(objTS.getValue())).fetchOne();
		return objDM;
	}
	...
	public DanhMuc getChuDeLienHe() {
		ThamSo objTS = find(ThamSo.class).where(QThamSo.thamSo.trangThai.ne(core().TT_DA_XOA))
				.where(QThamSo.thamSo.ma.eq(ThamSoEnum.CAT_ID_LIENHE)).fetchOne();
		DanhMuc objDM = find(DanhMuc.class).where(QDanhMuc.danhMuc.trangThai.ne(core().TT_DA_XOA))
				.where(QDanhMuc.danhMuc.id.eq(objTS.getValue())).fetchOne();
		return objDM;
	}
